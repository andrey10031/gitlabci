<template>
  <div class="cards__wrapper space-y-4">
    <h3 class="cards__title">From</h3>
    <div className="cards__fromTo">
      <div className="tokenNetwork sm:!max-w-[280px]">
        <span>Token</span>
        <SharedSelect :isSelect="true" label="NNI" />
      </div>
      <div className="tokenNetwork sm:!max-w-[160px]">
        <span>Network</span>
        <SharedSelect
          :options="select.getOptions"
          :label="select.getFrom.label.slice(0, 3)"
    ...
    <h3 class="cards__title !mt-6 sm:!mt-4">To</h3>
    <div className="cards__fromTo ">
      <div className="tokenNetwork sm:!max-w-[280px]">
        <span>Token</span>
        <SharedSelect :isSelect="false" label="NNI" />
      </div>
      <div className="tokenNetwork sm:!max-w-[160px]">
        <span>Network</span>
        <SharedSelect
          :options="select.getOptions"
          :label="select.getTo.label.slice(0, 3)"
  ...
  <!-- <div class="cards__wrapper">
    <h3 class="cards__title">From</h3>
    <div class="cards__header">
      <SharedSelect :options="select.getOptions" :label="select.getFrom.label" :imgPath="select.getFrom.img" :left="true" @select="select.fromHandler" />
...
import ToAddress from './ToAddress.vue'
...
const disconnectButton = computed(() => {
  if (
    (select.getFrom.label === 'Solana' && store.getWalletAddressFromSol) ||
    (select.getFrom.label === 'Binance Smart Chain' && store.getWalletAddressFromBe) ||
    (select.getFrom.label === 'Ethereum' && store.getWalletAddressFromBe)
  ) {
    return true
  } else {
    return false
